...
`approver_identity_drift` exists to leave an audit-trail comment whenever an
`Acceptance Approver` / `QA Approver` value is changed on a card that has
reached its review column — so a quietly-swapped approver on a reviewed card is
never silent. It decides relevance purely from the card's **current** `Status`
(`approver_identity_drift.py:9-27`): Acceptance-approver edits are logged only
while the card is in `User acceptance` / `QA approved` / `Released`, QA-approver
edits only in `QA approved` / `Released`.
...
> **Note (2026-06-01):** this plan was written before the board's `PQ review`
> step was renamed to `User acceptance` and its `PQ Approver` / `PQ Signoff
> Date` fields to `Acceptance Approver` / `Acceptance Signoff Date` (the
> "PQ-before-environment" fix). Identifiers below reflect the post-rename names.
...
- **`scripts/project_enforcement/checks/drift/approver_identity_drift.py`** —
  the check being changed. Note the `_ACCEPTANCE_STATUSES_OR_LATER` /
...
- **`scripts/project_enforcement/tests/test_drift_checks.py`** — the
  `TestApproverIdentityDrift` cases and the `_ctx(...)` / `_change(...)` helpers
  (lines 30-59). **`test_change_before_relevant_column_silent` encodes the blind
  spot as intended behaviour and must be flipped.**
...
- **`README.md`** (the "Field-drift" bullet) — documents this check; its wording
  ("cards already past their review column") goes stale once the trigger broadens.
...
1. **Broaden the relevance gate to `(status at/after review column) OR
   ...
   the `user_acceptance` / `qa_approved` preconditions). So a non-empty *previous*
   ...
   - `"alice" → ""` (clearing) at `In development` after move-back → `old_value`
     non-empty → **fires**.
   - `"" → "alice"` at `User acceptance` (normal first set) → status at column →
...
- [ ] **1a. MODIFY (tests):** `scripts/project_enforcement/tests/test_drift_checks.py`
  - **Flip** `test_change_before_relevant_column_silent`: rename to
    ...
    `Status: "In development"` now **emits one comment** naming `alice` and `bob`.
    This is the blind-spot fix; the old assertion encoded the bug.
  - **Add** `test_first_set_below_column_silent`: `_change("Acceptance Approver", "", "alice")`
  ...
  - Keep `test_change_after_acceptance_audit_logs` and `test_no_revert` green
    unchanged — they still hold under the broadened gate.
  - Note: the QA-approver case must respect its own column set — a `QA Approver`
    first-set (`"" → "carol"`) at `User acceptance` is *below* the QA column, so
...
- [ ] **1b. MODIFY:** `scripts/project_enforcement/checks/drift/approver_identity_drift.py`
  ...
  - Reword the comment body so it does not assert the card is currently past its
    column — e.g. header
    ...
    guidance.
  - Update the module docstring to state the check fires when an approver changes
    on a card that is at/after its review column **or** already carried an
    approver value (i.e. has been reviewed), and still does not revert.
...
- [ ] **MODIFY:** `README.md` (Field-drift bullet) — change the approver clause
  ...
  approver edited after the card is moved backward is still logged)". Keep it a
  single clause consistent with the surrounding list style.
