"""PQ review precondition.

PQ checklist ticked, PQ Approver set and valid, segregation of duties
against issue author and PR commit authors.
"""
...
def _parse_pq_checks(body: str) -> dict[str, bool]:
    parsed = dict(parse_checklist(body, "PQ review checklist:"))
    return {label: parsed.get(label, False) for label in PQ_CHECKLIST_LABELS}
...
def check(item_meta, ctx, evidence) -> list[str]:
    ...
    checks = _parse_pq_checks(issue.body)
    unticked = [label for label, ticked in checks.items() if not ticked]
    for label in unticked:
        reasons.append(f"PQ review checklist item not ticked: `{label}`.")

    approver = (fields.get("PQ Approver") or "").strip().lstrip("@")
    if not approver:
        reasons.append("`PQ Approver` field is unset.")
    else:
        if not evidence_user_exists(evidence, approver, ctx):
            reasons.append(f"`PQ Approver` `{approver}` is not a known GitHub user.")
        if approver == issue.author:
            reasons.append(
                f"`PQ Approver` (`{approver}`) is the issue author — segregation of duties."
            )
        ...
        if approver in commit_authors:
            reasons.append(
                f"`PQ Approver` (`{approver}`) authored commits on the linked PR — segregation of duties."
            )
