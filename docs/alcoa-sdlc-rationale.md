...
the branch-protection ruleset for `critical-trial` *and*
`trial-governance` / `personal-data` repos, rather than only at the
data layer?
...
| **Original** | First entry, or transcription? | Is this the original, untampered source code? Did the build pull from `main`? | `non_fast_forward`, `required_linear_history` (critical-trial), branch-deletion block |
...
enforced for `critical-trial` repos — but it is also why the same
rule applies to `trial-governance` and `personal-data` repos: the
...
the same demand for personal data, and trial-governance systems
(eTMF, CTMS, QMS) are themselves validated computerised systems
under ICH E6(R3).

## What this justifies in the rulesets

- `required_signatures` in **both** `cctc-critical-trial` and
